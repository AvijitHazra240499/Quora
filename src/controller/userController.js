...
try {
    ...
    let arr=["fname","lname","email","phone","password"]
    let regex={"email":/^\w+([\.-]?\w+)@\w+([\. -]?\w+)(\.\w{2,3})+$/,"phone":/^(\+\d{1,3}[- ]?)?\d{10}$/}
    let msg={"email":" email is invalid","phone":" phone is invalid,It should contain 10 number and number should start from 7 to 9"}
    let unique={"email":"userModel","phone":"userModel"}
